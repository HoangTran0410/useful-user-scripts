...
async function init() {
  for (let i = 0; i < modes.length; i++) {
    const response = await fetch(
      "https://api.n.m28.io/endpoint/digdig-" + modes[i] + "/findEach"
    );
...
Context.arc = new Proxy(Context.arc, {
  apply(target, ctx, [x, y, r]) {
    ...
    if (isRunning && ctx.fillStyle === "#222222" && x !== 0 && y !== 0) {
      ...
      const b =
        "in " + (mins > 0 ? mins + "m " : "") + (seconds % 60).toFixed(1) + "s";
  ...
  apply(target, thisArgs, [{ golds }]) {
    if (golds !== undefined) {
      ...
      for (let i = 0; i < golds.length; i++) {
        ...
        goldPositions.push([(m / chunkSize) * a + e, (n / chunkSize) * d + f]);
...
OffscreenContext.putImageData = new Proxy(OffscreenContext.putImageData, {
  apply(target, thisArgs, [{ data }]) {
    ...
    if (
      thisArgs.canvas.width === chunkSize &&
      thisArgs.canvas.height === chunkSize
    ) {
